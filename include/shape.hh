...
namespace RayTracer {
    ...
    public:
        ...
        constexpr float GetIntersectDistance() const
        {
            return t;
        }
    ...
    {
        return MathUtils::ApproxEqual(lhs.GetIntersectDistance(), rhs.GetIntersectDistance()) && lhs.shapePtr == rhs.shapePtr;
    }
    ...
    namespace IntersectionUtils {
        ...
        requires(std::is_same_v<std::decay_t<Args>, Intersection>&&...) constexpr auto AggregateIntersection(Args&&... args)
        {
            auto result = std::array{std::forward<Args>(args)...};
            std::sort(result.begin(), result.end(), [](const Intersection& lhs, const Intersection& rhs) -> bool {
        ...
        requires(std::is_same_v<typename IntersectionList::value_type, Intersection>) constexpr void SortIntersections(IntersectionList& intersections) noexcept
        {
            std::sort(intersections.begin(), intersections.end(), [](const Intersection& lhs, const Intersection& rhs) -> bool {
                // ignore all intersection with negative t (hit will never be behind the ray's origin)
                if (lhs.GetIntersectDistance() < 0)
                    return false;
                else if (rhs.GetIntersectDistance() < 0)
        ...
        requires(std::is_same_v<typename IntersectionList::value_type, Intersection>)
            [[nodiscard]] constexpr std::optional<Intersection> VisibleHit(IntersectionList intersections) noexcept
        {
            if (intersections.size() == 0)
                return std::nullopt;
            assert(std::is_sorted(intersections.begin(), intersections.end(), [](const Intersection& lhs, const Intersection& rhs) -> bool {
                if (lhs.GetIntersectDistance() < 0)
                    return false;
                else if (rhs.GetIntersectDistance() < 0)
                    return true;
                return lhs.GetIntersectDistance() < 0 && lhs.GetIntersectDistance() < rhs.GetIntersectDistance();
            }));
            return intersections[0].GetIntersectDistance() < 0 ? std::nullopt
    ...
    public:
        ...
        constexpr Shape(const Transform& transform_)
            : transform{transform_}
        {
        }

        constexpr Shape(const Material& material_, const Transform& transform_ = PredefinedMatrices::I<double, 4>)
            : material{material_}, transform{transform_}
        {
        }

        constexpr std::optional<Intersection> LocalIntersection(const Ray& ray, const ShapeWrapper* ptrSelf) const
        {
            return this->derived().LocalIntersection(ray, ptrSelf);
        }

        constexpr std::optional<Intersection> IntersectWith(const Ray& ray, const ShapeWrapper* ptrSelf) const
        {
    ...
    public:
        ...
        constexpr auto LocalIntersection(const Ray& ray, const ShapeWrapper* ptrSelf) const noexcept
        ...
        constexpr auto IntersectWith(const Ray& ray, const ShapeWrapper* ptrSelf) const noexcept
            -> StaticVector<Intersection, 2>
        {
    ...
    namespace ShapeTraits {
        template <ShapeType Tag>
        struct InstersectionTraits;

        template <>
        struct InstersectionTraits<SphereTag> {
            using return_type = StaticVector<Intersection, 2>;
            static constexpr int NumIntersections = 2;
        };
    ...
    public:
        ...
        constexpr ShapeType GetShapeType() const
        {
            return std::visit([&](auto const& elem) -> decltype(auto) { return elem.GetShapeType(); }, shapeObject);
        }

        constexpr Tuple GetWorldNormalAt(const Tuple& worldPoint) const
        {
            return std::visit([&](auto const& elem) -> decltype(auto) { return elem.WorldNormalAt(worldPoint); }, shapeObject);
        }

        constexpr auto IntersectWith(const Ray& ray) const
            -> std::variant<StaticVector<Intersection, 2>, StaticVector<Intersection, 1>>
        {
            return std::visit([&](auto const& elem) { return elem.IntersectWith(ray, this); },
                shapeObject);
        }
