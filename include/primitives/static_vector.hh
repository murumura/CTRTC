...
namespace RayTracer {
    ...
    public:
        ...
        requires(!std::is_same_v<InputIt, value_type>) constexpr StaticVector(const InputIt& first, const InputIt& last) noexcept
        {
            InputIt it = first;
            while (it != last) {
                push_back(*it);
        ...
        requires(std::is_same_v<Args, value_type>&&...) constexpr StaticVector(Args&&... args)
