...
namespace RayTracer {
    ...
    public:
        ...
        {
            static_assert(Length == 4, "Cross product can only be performed on vector type");

            return Vec{
                contents[TupleConstants::y] * rhs[TupleConstants::z] - contents[TupleConstants::z] * rhs[TupleConstants::y],
        ...
        friend std::ostream& operator<<(std::ostream& stream, const Vec& vec) noexcept
        {
            stream << "Vector(";
    ...
    */
    ...
    Vec(T, U...)
        -> Vec<std::enable_if_t<(std::is_same_v<T, U> && ...), T>,
            1 + sizeof...(U)>;
    ...
    template <typename T, std::size_t N>
    constexpr bool operator==(const Vec<T, N>& vecA, const Vec<T, N>& vecB) noexcept
    {
    ...
    namespace VecUtils {
        ...
        template <
            typename Func, typename T, typename... Vectors,
            typename ResultType = std::invoke_result_t<Func, T, typename Vectors::ValueType...>,
            std::size_t N = PrimitiveTraits::AllSameValue_v<std::size_t, Vectors::Length...>>
        constexpr Vec<ResultType, N> ElementWise(Func func, const Vec<T, N>& v, const Vectors&... vectors) noexcept
        {
    ...
    template <typename T, std::size_t N>
    constexpr Vec<T, N> operator+(const Vec<T, N>& vecA, const Vec<T, N>& vecB) noexcept
    {
        return VecUtils::ElementWise(std::plus<T>(), vecA, vecB);
    }

    template <typename T, std::size_t N>
    constexpr Vec<T, N>& operator+=(Vec<T, N>& vecA, const Vec<T, N>& vecB) noexcept
    {
        for (std::size_t i = 0; i < N; ++i)
            vecA[i] += vecB[i];
        return vecA;
    ...
    */
    ...
    {
        return Tuple{
            tuple[TupleConstants::x] * -1.0,
            tuple[TupleConstants::y] * -1.0,
            tuple[TupleConstants::z] * -1.0,
    ...
    {
        return Tuple{
            tuple[TupleConstants::x] + scalar,
            tuple[TupleConstants::y] + scalar,
            tuple[TupleConstants::z] + scalar,
            tuple[TupleConstants::w]};
    }
    ...
    {
        return Tuple{
            tuple[TupleConstants::x] + rhs[TupleConstants::x],
    ...
    template <>
    constexpr Tuple::ValueType
    Tuple::DotProduct(const Tuple& rhs) const noexcept
    {
        ValueType result{};
        result += ((contents[TupleConstants::x] * rhs[TupleConstants::x]) + (contents[TupleConstants::y] * rhs[TupleConstants::y]) + (contents[TupleConstants::z] * rhs[TupleConstants::z]));
        return result;
    }

    constexpr Tuple ToVector(const Tuple& tuple)
    {
        return MakeVector(tuple[TupleConstants::x], tuple[TupleConstants::y], tuple[TupleConstants::z]);
    }

    constexpr Tuple ToNormalizedVector(const Tuple& tuple)
    {
        return MakeVector(tuple[TupleConstants::x], tuple[TupleConstants::y], tuple[TupleConstants::z]).Normalize();
    }
    ...
    struct PredefinedTuples {
        ...
        static constexpr Tuple MinPoint{MathUtils::MathConstants::INF<>, MathUtils::MathConstants::INF<>, MathUtils::MathConstants::INF<>, TupleConstants::PointFlag};
