...
requires(std::is_arithmetic_v<T>) class Matrix {
 public:
  ...
  constexpr Vec<T, Cols> Row(std::size_t rowIndex) const {
    ...
    return VecUtils::Generate<Cols>(
        [rowIndex, this](std::size_t j) { return contents[rowIndex][j]; });
  }

  constexpr Vec<T, Rows> Column(std::size_t colIndex) const {
    if (colIndex >= Cols)
      throw "index out of range";
    return VecUtils::Generate<Rows>(
        [colIndex, this](std::size_t j) { return contents[j][colIndex]; });
  }
  ...
  /* Returns a pointer to the specified row
         ...
         * row pointer.
         */
  constexpr T const* operator[](std::size_t rowIndex) const {
...
template <typename Func, typename T, typename... Matrices,
          typename ResultType =
              std::invoke_result_t<Func, T, typename Matrices::ValueType...>,
          std::size_t C =
              PrimitiveTraits::AllSameValue_v<std::size_t, Matrices::Cols...>,
          std::size_t R =
              PrimitiveTraits::AllSameValue_v<std::size_t, Matrices::Rows...>>
constexpr Matrix<T, R, C> ElementWise(Func func, const Matrix<T, R, C>& mat,
                                      const Matrices&... matrices) noexcept {
  Matrix<ResultType, R, C> opApplied = {};
  for (std::size_t i = 0; i < R; ++i) {
    for (std::size_t j = 0; j < C; ++j) {
      opApplied[i][j] =
          std::apply(func, std::forward_as_tuple(mat[i][j], matrices[i][j]...));
    }
  }
  return opApplied;
}
...
template <typename T, std::size_t R, std::size_t C>
constexpr bool operator==(const Matrix<T, R, C>& matA,
                          const Matrix<T, R, C>& matB) noexcept {
  for (std::size_t i = 0; i < R; ++i) {
    for (std::size_t j = 0; j < C; ++j) {
...
constexpr Matrix<T, R, C> operator+(T scalar,
                                    const Matrix<T, R, C>& mat) noexcept {
  return MatrixUtils::ElementWise([scalar](T x) { return x + scalar; }, mat);
}

template <typename T, std::size_t R, std::size_t C>
constexpr Matrix<T, R, C> operator+(const Matrix<T, R, C>& matA,
                                    const Matrix<T, R, C>& matB) noexcept {
...
template <typename T, std::size_t R, std::size_t C>
constexpr Matrix<T, R, C>& operator+=(Matrix<T, R, C>& matA,
                                      const Matrix<T, R, C>& matB) noexcept {
  for (std::size_t row = 0; row < R; row++) {
    for (std::size_t col = 0; col < C; col++) {
...
constexpr Matrix<T, R1, C2> operator*(const Matrix<T, R1, C>& matA,
                                      const Matrix<T, C, C2>& matB) noexcept {
  Matrix<T, R1, C2> ret{};
...
constexpr Matrix<T, D, D>& operator*=(Matrix<T, D, D>& matA,
                                      const Matrix<T, D, D>& matB) noexcept {
...
// mat * vec
template <typename T, std::size_t R, std::size_t C>
constexpr Vec<T, C> operator*(const Matrix<T, R, C>& mat,
                              const Vec<T, C>& vec) noexcept {
  Vec<T, C> ret{};
  for (std::size_t row = 0; row < R; row++) {
    for (std::size_t col = 0; col < C; col++) {
...
constexpr Matrix<T, C, R> Transpose(const Matrix<T, R, C>& mat) {
  return MatrixUtils::Generate<C, R>(
      [&mat](auto i, auto j) { return mat[j][i]; });
}

template <typename T, std::size_t R, std::size_t C>
constexpr Matrix<T, R - 1, C - 1> SubMatrix(const Matrix<T, R, C>& mat,
                                            std::size_t omitRow,
                                            std::size_t omitCol) {
...
template <typename T, std::size_t N>
constexpr T Minor(const Matrix<T, N, N>& mat, std::size_t omitRow,
                  std::size_t omitCol) {
  return Determinant(SubMatrix(mat, omitRow, omitCol));
}

template <typename T, std::size_t N>
constexpr T Cofactor(const Matrix<T, N, N>& mat, std::size_t omitRow,
                     std::size_t omitCol) {
