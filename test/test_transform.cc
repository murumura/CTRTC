...
TEST(Transform, translation)
{
    ...
    ASSERT_EQ(translation * p, MakePoint(2.0, 1.0, 7.0));
...
TEST(Transform, scale)
{
    ...
    ASSERT_EQ(invScale * v1, expectedV2);
    ...
    ASSERT_EQ(reflection * p2, expectedP2);
}
...
TEST(Transform, rotationX)
{
    ...
    ASSERT_EQ(halfQuarterX * p1, expectedP1);

    constexpr auto fullQuarterX = MatrixUtils::RotateX(piRad / 2.0);
    ASSERT_EQ(fullQuarterX * p1, MakePoint(0.0, 0.0, 1.0));
    ...
    ASSERT_EQ(invHalfQuarterX * p1, invExpectedP);
}
...
TEST(Tranform, rotationZ)
{
    ...
    ASSERT_EQ(halfQuarterZ * p1, expectedP1);

    constexpr Tuple expectedP2 = MakePoint(-1, 0, 0);
    constexpr auto fullQuarterZ = MatrixUtils::RotateZ(piRad / 2);

    ASSERT_EQ(fullQuarterZ * p1, expectedP2);
}
...
TEST(Transform, shearing)
{
    ...
    ASSERT_EQ(shear1 * p1, expectedP1);
    ...
    ASSERT_EQ(shear2 * p1, expectedP2);
    ...
    ASSERT_EQ(shear3 * p1, expectedP3);
    ...
    ASSERT_EQ(shear4 * p1, expectedP4);
    ...
    ASSERT_EQ(shear5 * p1, expectedP5);
}
...
TEST(Transform, move_the_world)
{
    ...
    constexpr auto t = MatrixUtils::ViewTransform(from, to, up);
    ASSERT_EQ(t, MatrixUtils::Translation(0, 0, -8));
}
...
TEST(Transform, chaining)
{
    ...
    ASSERT_EQ(p1, MakePoint(1, -1, 0));
    constexpr auto p2 = B * p1;
    ASSERT_EQ(p2, MakePoint(5, -5, 0));
    constexpr auto p3 = C * p2;
    ASSERT_EQ(p3, MakePoint(15, 0, 7));

    // Chained transformations must be applied in reverse order
    constexpr auto T1 = C * B * A;
    ASSERT_EQ(T1 * p, MakePoint(15, 0, 7));
