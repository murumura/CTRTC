...
TEST(Transform, translation) {
  ...
  constexpr TransformMatrix expected = {1, 0, 0, 5, 0, 1, 0, -3,
                                        0, 0, 1, 2, 0, 0, 0, 1};
...
TEST(Transform, rotationX) {
  ...
  constexpr auto expectedP1 = MakePoint(0.0, MathUtils::ConstExprSqrtf(2.0) / 2,
                                        MathUtils::ConstExprSqrtf(2.0) / 2);
  ...
  constexpr auto invExpectedP =
      MakePoint(0.0, MathUtils::ConstExprSqrtf(2.0) / 2,
                -MathUtils::ConstExprSqrtf(2.0) / 2);
...
TEST(Transform, rotationY) {
  ...
  constexpr Tuple expectedP1 = MakePoint(MathUtils::ConstExprSqrtf(2.0) / 2, 0,
                                         MathUtils::ConstExprSqrtf(2.0) / 2);
...
TEST(Tranform, rotationZ) {
  ...
  constexpr Tuple expectedP1 = MakePoint(-MathUtils::ConstExprSqrtf(2.0) / 2,
                                         MathUtils::ConstExprSqrtf(2.0) / 2, 0);
...
TEST(Transform, arbitary_transform) {
  ...
  constexpr auto expected =
      TransformMatrix{-0.50709, 0.50709,  0.67612,  -2.36643, 0.76772,  0.60609,
...
TEST(Transform, chaining) {
  ...
  constexpr auto T2 =
      MatrixUtils::RotateX(piRad / 2.0).Scale(5, 5, 5).Translation(10, 5, 7);
  EXPECT_EQ(T1, T2);
