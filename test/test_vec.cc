...
TEST(Vec, utils) {
  ...
  EXPECT_EQ(seqVec, seqVecExpected);
  constexpr auto genVecExpected = Vec{0.0, 1.0, 4.0, 9.0};
  constexpr auto genVec =
      VecUtils::Generate<4>([](auto i) { return double(i * i); });
  EXPECT_EQ(genVec, genVecExpected);
}
...
TEST(Vec, constructor) {
  ...
  static_assert(CreateVector(1.0, 2.0, 3.0) == Vec{1.0, 2.0, 3.0},
                "MakeVector and uniform initialization deduction guide");
  static_assert(CreateVector(1.0, 2.0, 3.0) == Vec{{1.0, 2.0, 3.0}},
                "MakeVector and uniform initialization deduction guide");
}
...
TEST(Vec, opeartor_plus) {
  ...
  static_assert(Vec{1.0, 2.0, 3.0} + Vec{2.0, 2.0, 2.0} == Vec{3.0, 4.0, 5.0},
                "operator+");
...
TEST(Vec, opeartor_minus) {
  ...
  static_assert(
      (Vec{1.0, 2.0, 3.0} - Vec{2.0, 2.0, 2.0}) == Vec{-1.0, 0.0, 1.0},
      "operator-");
  constexpr auto v1 = Vec{1.0, 2.0, 3.0} - 2.0;
...
TEST(Vec, opeartor_mul) {
  ...
  static_assert((Vec{1.0, 2.0, 3.0} * Vec{2.0, 2.0, 2.0}) == Vec{2.0, 4.0, 6.0},
                "operator*");
  constexpr auto v1 = Vec{1.0, 2.0, 3.0} * 2.0;
...
TEST(Vec, opeartor_div) {
  ...
  static_assert((Vec{2.0, 4.0, 6.0} / Vec{2.0, 2.0, 2.0}) == Vec{1.0, 2.0, 3.0},
                "operator/");
...
TEST(Tuple, constructor) {
  ...
  EXPECT_TRUE(vec1 == MakeVector(0, 1, 2));
  ...
  EXPECT_EQ(pt1.size(), 4);
  EXPECT_TRUE(pt1 == pt2);
  EXPECT_TRUE(pt1 == MakePoint(1, 2, 3));

  EXPECT_FALSE(vec1 == MakePoint(1, 2, 3));
}
...
TEST(Tuple, opeartor_negate) {
  ...
  EXPECT_EQ(-vec1, vec2);
  EXPECT_EQ(-1.0 * vec1, vec2);
  EXPECT_EQ(vec1 * -1.0, vec2);

  constexpr auto pt1 = MakePoint(1.0, 2.0, 3.0);
  constexpr auto pt2 = MakePoint(-1.0, -2.0, -3.0);
  EXPECT_EQ(-pt1, pt2);
  EXPECT_EQ(-1.0 * pt1, pt2);
  EXPECT_EQ(pt1 * -1.0, pt2);
}
...
TEST(Tuple, cross_product) {
  ...
  EXPECT_TRUE(z == PredefinedTuples::ZeroVector);
  ...
  EXPECT_FALSE(v2.CrossProduct(v1) == v1.CrossProduct(v2));
...
TEST(Tuple, reflect) {
  ...
  EXPECT_EQ(r, MakeVector(1.0, 1.0, 0.0));
  ...
  EXPECT_EQ(r2, MakeVector(1.0, 0.0, 0.0));
}
...
TEST(Tuple, dot_product) {
  ...
  EXPECT_TRUE(vec1.DotProduct(vec2) == 28);

  constexpr Tuple pt1{1, 2, 3, TupleConstants::PointFlag};
  constexpr Tuple pt2{2, 4, 6, TupleConstants::PointFlag};
  EXPECT_TRUE(pt1.DotProduct(pt2) == 28);
}
...
TEST(Tuple, point_to_vector) {
  ...
  EXPECT_EQ(vxExpected, vx);

  auto vxNorm = ToNormalizedVector(px);
  auto vxExpectedNorm = MakeNormalizedVector(1.0, 2.0, 3.0);
  EXPECT_EQ(vxExpectedNorm, vxNorm);

  auto vxy = ToVector(py - px);
  auto vxyExpected = MakeVector(-2.0, -4.0, -6.0);
  EXPECT_EQ(vxyExpected, vxy);
}

TEST(Colour, colour_are_rgb_tuple) {
  constexpr Colour rgb{-0.5, 0.4, 1.7};
  EXPECT_TRUE(MathUtils::ApproxEqual(rgb[ColourConstants::r], -0.5));
  EXPECT_TRUE(MathUtils::ApproxEqual(rgb[ColourConstants::g], 0.4));
  EXPECT_TRUE(MathUtils::ApproxEqual(rgb[ColourConstants::b], 1.7));
}
...
TEST(Colour, valid) {
  ...
  EXPECT_TRUE(IsValidColour(validC));
  constexpr Colour invalidC{1.2, -0.2, 0.4};
  EXPECT_FALSE(IsValidColour(invalidC));
}
